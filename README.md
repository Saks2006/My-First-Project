# 👋 Hi, I'm Saikat Sarkar

## 📍 Location
Balangir, Odisha, India | Patna, Bihar

## 💼 Professional Background
- **Field**: Telecommunications & Infrastructure Project Management
- **Expertise**: Fibre optics, 4G site maintenance, network infrastructure
- **Collaboration**: BSNL, BBNL teams
- **Focus**: Fault management, equipment approval, technical documentation

## 🎯 Learning Goals
- 🤖 AI & Machine Learning
- 📊 GitHub for AI projects
- 🔒 CISA Certification (Information Security)
- 📝 Technical report writing

## 🛠️ Skills
- **Tools**: Microsoft Excel, PDF/DOCX editors
- **Languages**: English, Odia
- **Technical**: Project management, optical fibre networks, formal communication

## 📚 Currently Learning
- GitHub fundamentals & version control
- GitHub Copilot (AI coding assistant)
- AI Engineering & LLMs

## 🔄 My GitHub Journey
- 📖 Learning: Microsoft's [AI for Beginners](https://github.com/microsoft/ai-for-beginners) [web:1]
- 🤖 Exploring: [GitHub Copilot Fundamentals](https://learn.microsoft.com/en-us/training/paths/copilot/) [web:11]
- 📂 Following: [Learn AI Engineering](https://github.com/ashishps1/learn-ai-engineering) [web:5]

## 📬 Connect
- 💼 LinkedIn: [Add your LinkedIn URL]
- 📧 Email: [Your email]
- 🌐 Portfolio: [Optional]

---

*Last updated: May 29, 2026*